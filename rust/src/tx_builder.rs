        ...
        match &input_total.partial_cmp(&output_total.checked_add(&Value::new(&fee))?) {
            ...
            Some(Ordering::Greater) => {
                ...
                if has_assets(change_estimator.multiasset()) {
                    ...
                    fn pack_nfts_for_change(max_value_size: u32, change_address: &Address, change_estimator: &Value, data_hash: Option<DataHash>) -> Result<Vec<MultiAsset>, JsError> {
                    ...
                    while let Some(Ordering::Greater) = change_left.multiasset.as_ref().map_or_else(|| None, |ma| ma.partial_cmp(&MultiAsset::new())) {
                        let nft_changes = pack_nfts_for_change(self.config.max_value_size, address, &change_left, data_hash.clone())?;
