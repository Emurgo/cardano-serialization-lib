    ...
    fn serialize<'se, W: Write>(
        ...
        let serializer = match &self.network_magic {
            ...
            &Some(network_magic) => serializer
                ...
                .write_bytes(cbor_event::cbor!(&network_magic)?)?,
        };
        Ok(serializer)
    }
}
...
fn hash_spending_data(xpub: &XPub, attrs: &Attributes) -> [u8; 28] {
    let buf = cbor_event::cbor!(&(&PubKeyTag(), &SpendingData(xpub), attrs))
...
impl ExtendedAddr {
    ...
    pub fn to_address(&self) -> Addr {
        Addr(cbor_event::cbor!(self).unwrap()) // unwrap should never fail from strongly typed extended addr to addr
    }
}
#[derive(Debug)]
