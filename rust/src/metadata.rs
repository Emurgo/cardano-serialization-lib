        ...
        (|| -> Result<_, DeserializeError> {
            match raw.cbor_type()? {
                ...
                CBORType::Map => Ok(TransactionMetadata {
                    general: GeneralTransactionMetadata::deserialize(raw)
                        .map_err(|e| e.annotate("general"))?,
                    native_scripts: None,
                }),
...
mod tests {
    ...
    fn json_encoding_no_conversions() {
        ...
        let tags_i32 = tags
            .0
            .iter()
            .map(|md| md.as_int().unwrap().as_i32().unwrap())
            .collect::<Vec<i32>>();
    ...
    fn json_encoding_check_example_metadatum(metadata: &TransactionMetadatum) {
        ...
        assert_eq!(
            map.get_i32(9).unwrap().as_int().unwrap().as_i32().unwrap(),
            5
        );
        ...
        assert_eq!(
            a1.get_i32(5).unwrap().as_int().unwrap().as_i32().unwrap(),
            2
        );
    ...
    fn json_encoding_detailed_complex_key() {
        ...
        assert_eq!(
            map.get(&key).unwrap().as_int().unwrap().as_i32().unwrap(),
            5
        );
